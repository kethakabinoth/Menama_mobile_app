  ...
  const applyFilter = useCallback(() => {
    ...
    if (searchText.trim()) {
      ...
      filtered = filtered.filter((o) =>
        ...
        `${o.Supplier_Name ?? o.Technician_Name ?? o.Acc_Name ?? ""}`.toLowerCase().includes(query)
      );
    }
    if (dateFilter) {
      const dateString = dateFilter.toISOString().split("T")[0];
      filtered = filtered.filter((o) => `${o.Tr_Date ?? ""}`.includes(dateString));
    }
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardHeader}>
          <View style={{ flex: 1 }}>
            <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 4 }}>
              <Text style={styles.orderNo}>{item.Pay_No || item.Voucher_No}</Text>
              <View style={[styles.methodBadge, { backgroundColor: method.bg, marginLeft: 8 }]}>
                <Text style={[styles.methodBadgeText, { color: method.color }]}>{method.label}</Text>
              </View>
            </View>
        ...
        <View style={styles.cardInfoRow}>
          <View style={styles.infoCol}>
            ...
            <Text style={styles.infoValue}>{item.Pay_Type || item.Tr_Type || "General"}</Text>
          </View>
          ...
          <View style={[styles.infoCol, { alignItems: "flex-end" }]}>
            ...
            <Text style={[styles.infoValue, { color: G.dark, fontWeight: "700" }]}>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.tabBar}>
        {(["Supplier", "Technician", "General"] as PaymentType[]).map((t) => {
          const Icon = t === "Supplier" ? ShoppingBag : t === "Technician" ? User : CreditCard;
      ...
      {dateFilter && (
        <View style={styles.activeDateBadge}>
          <Text style={styles.activeDateText}>📅 {formatDate(dateFilter.toISOString())}</Text>
      ...
      {loading && !refreshing ? (
        <View style={styles.centered}>
          ...
          <Text style={{ color: G.muted, marginTop: 10, fontSize: 13 }}>Loading payments…</Text>
        </View>
      ) : (
        <FlatList
          data={pagedPayments}
          renderItem={renderItem}
          keyExtractor={(item, index) => `${activeTab}-${item.Pay_No || item.Voucher_No || index}`}
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>No {activeTab.toLowerCase()} payments found</Text>
            </View>
          }
          ListFooterComponent={
            totalPages > 1 ? (
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalGreenHeader}>
              ...
              <View style={styles.modalHeaderRow}>
                <Text style={styles.modalTitle}>{activeTab} Payment Review</Text>
              ...
              <View style={styles.modalInfoCard}>
                ...
                <Text style={styles.modalInfoValue}>
                  {selectedPayment?.Supplier_Name || selectedPayment?.Technician_Name || selectedPayment?.Acc_Name}
                </Text>
                ...
                <Text style={styles.modalAmount}>
                  {formatCurrency(selectedPayment?.Amount || selectedPayment?.Tobe_Paid || 0)}
                </Text>
              </View>
            </View>
            ...
            <ScrollView style={styles.modalScroll} showsVerticalScrollIndicator={false}>
              ...
              {activeTab === "Technician" && (
                <View style={styles.techRow}>
                  <View style={styles.techCell}>
                    ...
                    <Text style={styles.techValue}>{selectedPayment?.S_Order || "N/A"}</Text>
                  </View>
                  <View style={styles.techCell}>
                    <Text style={styles.techLabel}>Tr. Date</Text>
                    <Text style={styles.techValue}>{formatDate(selectedPayment?.Tr_Date)}</Text>
                  </View>
                  <View style={styles.techCell}>
                    <Text style={styles.techLabel}>Tr. Type</Text>
                    <Text style={styles.techValue}>{selectedPayment?.Tr_Type || "N/A"}</Text>
                  </View>
                </View>
              )}
                  ...
                  {details.map((d: any, i: number) => (
                    <View key={i} style={styles.chequeCard}>
                      ...
                      <View style={styles.chequeRow}>
                        ...
                        {!!d.DOR && (
                          <View style={{ alignItems: "flex-end" }}>
                            <Text style={[styles.chequeLabel, { color: "#ef810c" }]}>Date Realized On</Text>
                            <Text style={[styles.chequeValue, { color: G.red }]}>
                              {formatDate(d.DOR)}
                            </Text>
                          </View>
                        )}
              ...
              <View style={styles.awaitingNotice}>
                ...
                <Text style={styles.awaitingText}>This payment is awaiting your approval</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  // Search
  searchBarContainer: {
    flexDirection: "row", paddingHorizontal: 14, paddingVertical: 12, gap: 10, alignItems: "center",
  },
  searchInputWrap: {
  ...
  activeDateBadge: {
    ...
    marginHorizontal: 14, marginBottom: 4, marginTop: -4,
  ...
  payeeName: { fontSize: 12, color: G.sub, marginTop: 2 },
  statusBadge: {
  ...
  infoCol: { flex: 1 },
  infoLabel: { fontSize: 9, color: G.muted, fontWeight: "600", marginBottom: 3 },
  ...
  modalInfoLabel: {
    ...
    fontWeight: "600", letterSpacing: 0.4, marginBottom: 2,
  },
  modalInfoValue: { fontSize: 14, fontWeight: "600", color: G.white, marginBottom: 8 },
  pendingBadge: {
    backgroundColor: "rgba(255,255,255,0.2)",
    borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4,
  },
  pendingBadgeText: { fontSize: 11, color: G.white, fontWeight: "600" },
  modalDivider: { height: 0.5, backgroundColor: "rgba(255,255,255,0.2)", marginVertical: 10 },
  modalAmountLabel: {
    fontSize: 9, color: "rgba(255,255,255,0.55)",
    fontWeight: "600", letterSpacing: 0.4, marginBottom: 3,
  },
  ...
  techRow: {
    ...
    backgroundColor: G.faint, borderRadius: 12,
    borderWidth: 1, borderColor: G.border, padding: 12,
  },
  techCell: { flex: 1 },
  techLabel: { fontSize: 10, color: G.muted, fontWeight: "600", marginBottom: 2 },
  techValue: { fontSize: 12, fontWeight: "600", color: G.text },
  ...
  chequeLabel: { fontSize: 15, color: "#81C784", fontWeight: "800", marginBottom: 3 },
  ...
  awaitingNotice: {
    ...
    borderWidth: 1, borderColor: G.redPale,
