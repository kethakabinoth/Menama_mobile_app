...
console.log(`Connecting to Cloud SQL Server at ${dbConfig.server}:${dbConfig.port}...`);
...
app.post('/logout', authenticateToken, async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query('INSERT INTO User_Logging (Username, Action) VALUES (@username, @action)');
    ...
    try {
        ...
        try {
            await transaction.request()
                ...
                .query("UPDATE Quatation SET Status = 'Approved' WHERE S_Order = @sOrder");
            
            await transaction.request()
                .input('sOrder', mssql.VarChar, sOrder)
                .query("UPDATE New_Sales_Order SET Quatation_Status = 'Approved' WHERE S_Order = @sOrder");
...
app.put('/quotations/:id/reject', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            await transaction.request()
                .input('sOrder', mssql.VarChar, sOrder)
                .query("UPDATE Quatation SET Status = 'Rejected' WHERE S_Order = @sOrder");
            
            await transaction.request()
                .input('sOrder', mssql.VarChar, sOrder)
                .query("UPDATE New_Sales_Order SET Quatation_Status = 'Rejected' WHERE S_Order = @sOrder");
...
app.put('/costings/:id/approve', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            await transaction.request()
                .input('sOrder', mssql.VarChar, sOrder)
                .query("UPDATE Pre_Costing SET Status = 'Approved' WHERE S_Order = @sOrder");
            
            await transaction.request()
                .input('sOrder', mssql.VarChar, sOrder)
                .query("UPDATE New_Sales_Order SET Costing_Status = 'Approved' WHERE S_Order = @sOrder");
...
app.put('/costings/:id/reject', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            await transaction.request()
                .input('sOrder', mssql.VarChar, sOrder)
                .query("UPDATE Pre_Costing SET Status = 'Rejected' WHERE S_Order = @sOrder");
            
            await transaction.request()
                .input('sOrder', mssql.VarChar, sOrder)
                .query("UPDATE New_Sales_Order SET Costing_Status = 'Rejected' WHERE S_Order = @sOrder");
...
app.put('/sales-orders/:id/approve-costing', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
            .query("UPDATE New_Sales_Order SET Costing_Status = 'Approved' WHERE ID = @id");
...
app.put('/sales-orders/:id/reject-costing', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
            .query("UPDATE New_Sales_Order SET Costing_Status = 'Rejected' WHERE ID = @id");
...
app.put('/sales-orders/:id/approve-quotation', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
            .query("UPDATE New_Sales_Order SET Quatation_Status = 'Approved' WHERE ID = @id");
...
app.put('/sales-orders/:id/reject-quotation', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
            .query("UPDATE New_Sales_Order SET Quatation_Status = 'Rejected' WHERE ID = @id");
...
app.get('/dashboard', authenticateToken, async (req, res) => {
    try {
        ...
        const costingCount = await pool.request().query("SELECT COUNT(*) as count FROM Pre_Costing WHERE LTRIM(RTRIM(Status)) = 'A'");
        // Open Quotations from Quatation table
        const quotationCount = await pool.request().query("SELECT COUNT(*) as count FROM Quatation WHERE LTRIM(RTRIM(Status)) = 'A'");

        // Pending Payments aggregation
        const supplierPayCount = await pool.request().query("SELECT COUNT(*) as count FROM Supplier_payment_H WHERE Approvel = 'READY'");
        const techPayCount = await pool.request().query("SELECT COUNT(*) as count FROM Outside_Technician_Pay WHERE Status = 'READY'");
        const voucherPayCount = await pool.request().query("SELECT COUNT(*) as count FROM Payment_Voucher WHERE Status = 'READY'");

        // Pending Cheques aggregation
        const supplierChqCount = await pool.request().query("SELECT COUNT(*) as count FROM Supplier_Payment_Cheq_D");
        const voucherChqCount = await pool.request().query("SELECT COUNT(*) as count FROM Payment_Voucher_Chq");
        ...
        const summary = {
            ...
            PendingPayments: (supplierPayCount.recordset[0].count || 0) + (techPayCount.recordset[0].count || 0) + (voucherPayCount.recordset[0].count || 0),
            PendingCheques: (supplierChqCount.recordset[0].count || 0) + (voucherChqCount.recordset[0].count || 0),
        ...
        // Approved History - latest 10 approved items (either costing or quotation)
        const history = await pool.request().query(`
            SELECT TOP 10 
                ID, S_Order, Customer_Name, Product_Name, Rate, Tr_Date,
                CASE 
                    WHEN Costing_Status = 'Approved' AND Quatation_Status = 'Approved' THEN 'Both'
                    WHEN Costing_Status = 'Approved' THEN 'Costing'
                    WHEN Quatation_Status = 'Approved' THEN 'Quotation'
                END as ApprovedType
            FROM New_Sales_Order
            WHERE LTRIM(RTRIM(Status)) = 'A' 
              AND (Costing_Status = 'Approved' OR Quatation_Status = 'Approved')
            ORDER BY Tr_Date DESC
        `);

        res.json({
            summary: summary,
            outstanding: outstandingStats.recordset[0] || { TotalNet: 0, TotalPaid: 0, TotalBalance: 0 },
...
app.get('/supplier-payments/:payNo/details', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        const result = await pool.request()
            .input('payNo', mssql.VarChar, req.params.payNo)
            .query("SELECT * FROM Supplier_Payment_Cheq_D WHERE LTRIM(RTRIM(Pay_No)) = @payNo");
        res.json(result.recordset);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
app.put('/supplier-payments/:id/approve', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
            .query("UPDATE Supplier_payment_H SET Approvel = 'Approved' WHERE ID = @id");
...
app.put('/supplier-payments/:id/reject', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
            .query("UPDATE Supplier_payment_H SET Approvel = 'Rejected' WHERE ID = @id");
...
app.get('/api/supplier-payments/pending-count', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        const result = await pool.request().query("SELECT COUNT(*) as count FROM Supplier_payment_H WHERE Approvel = 'READY'");
...
app.get('/api/supplier-payments/cheque-count', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        const result = await pool.request().query("SELECT COUNT(*) as count FROM Supplier_Payment_Cheq_D");
        res.json({ count: result.recordset[0].count });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
app.put('/technician-payments/:id/approve', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
            .query("UPDATE Outside_Technician_Pay SET Status = 'Approved' WHERE ID = @id");
...
app.put('/technician-payments/:id/reject', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
            .query("UPDATE Outside_Technician_Pay SET Status = 'Rejected' WHERE ID = @id");
...
app.get('/voucher-payments/:voucherNo/details', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        const result = await pool.request()
            .input('voucherNo', mssql.NChar, req.params.voucherNo)
            .query("SELECT * FROM Payment_Voucher_Chq WHERE LTRIM(RTRIM(Pay_No)) = @voucherNo");
        res.json(result.recordset);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
app.put('/voucher-payments/:id/approve', authenticateToken, async (req, res) => {
    try {
        const pool = await poolPromise;
        await pool.request()
            .input('id', mssql.Int, req.params.id)
